...
TEST_CASE("test query_and - true true case") {
	auto subq = query_equals("abc", "123");
	auto q = query_and(subq, subq);
...
TEST_CASE("test query_and - true false case") {
	auto subq1 = query_equals("abc", "123");
	auto subq2 = query_equals("abc", "456");
	auto q = query_and(subq1, subq2);
...
TEST_CASE("test query_and - false true case") {
