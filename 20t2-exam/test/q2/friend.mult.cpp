...
TEST_CASE("Multiplication binds to sparse_matrix&") {
...
TEST_CASE("Multiplication binds to sparse_matrix&&") {
	...
	auto result = make_sparse_matrix<long long>(2, 2, {{0, 0, 2}, {1, 1, 2}})
	              * make_sparse_matrix<long long>(2, 3, {{0, 0, 1}, {1, 2, 1}});
...
TEST_CASE("Exception is thrown as exepcted") {
	...
	CHECK_THROWS_MATCHES(lhs * rhs, matrix_error, Catch::Matchers::Message("LHS cols() != RHS rows()"));
}
